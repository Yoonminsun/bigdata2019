...
def getPostData(post,jsonResult):
    ...
    if post['link'].find('logNo')!=-1:
        p = re.compile('.+://\w+.\w+(?!/).?\w*(?!/).?\w*')
        m = p.search(post['link'])
        p = re.compile('.+logNo=(\d+)')
        n = p.sub('\g<1>', post['link'])
        link = m.group()+'/'+n
    else :
        link = post['link']
    pDate = post['postdate']

    jsonResult.append({'name':name,'title':title,'description':description,'link':link,'pDate':pDate,'bloggerlink':bloggerlink})
    return
