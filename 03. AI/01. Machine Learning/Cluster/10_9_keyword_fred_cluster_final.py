...
# 데이터 구조 정의
# 사용자 ID를 key로, 상품 코드의 셋을 value로 갖는 dictionary
user_product_dic = {}
# 상품 코드를 key로, 사용자  ID의 셋을 value로 갖는 dictionary
product_user_dic = {}

# 상품 코드를 key로, 상품명을 value로 갖는 dictionary
# 군집화의 내용을 확인하는 단계에서 상품명을 사용한다.
product_id_name_dic = {}

def analyze_clusters_keywords(labels, product_id_name_dic, user_product_dic, id_user_dic):
    # 각 클러스터의 ID와, 해당 ID의 클러스터에 들어있는 유저 수를 출력
    ...
    for i in range(len(labels)):
        ...
        # 각 사용자의 임시 ID, i 에 대해 사용자 코드를 찾은 후
        # 그 사용자 코드와 연결된 구매상품의 ID를 참조한 후
        # 그 ID를 이용해 상품명을 찾아
        # dictionary에 클러스터 ID를 key로, 상품명을 value로 추가
    ...
    for cluster_id, product_name in cluster_item.items():
        # 각 클러스터 안의 상품명을 join 으로 합쳐 하나의 문자열로 만든 후
        # 스페이스 또는 탭으로 split 하여 키워드로 분해
        product_name_keyword = (" ").join(product_name).split()

        # 클러스터의 ID와, 그 ID를 가지는 클러스터에 속하는 유저들이 구매한 상품들의 상품명안에
...
def analyze_clusters_keywords_bigram(labels,product_id_name_dic,user_product_dic,id_user_dic):
    ...
    for cluster_id, product_name in cluster_item.items():
        # 각 클러스터 안의 상품명을 join 으로 합쳐 하나의 문자열로 만든 후
        # OF를 공백으로 replace 하고
        # 스페이스 또는 탭으로 split 하여 키워드로 분해한 후
...
for line in open('online_retail_utf.txt'):
    ...
    # 읽은 정보로 데이터 구조를 채움
    # 상품 가짓수를 고려하므로 상품 코드를 셋으로 가지도록 할 예정 (set으로 중복 방지)
...
# kurtosis : 정규 분포상에서 값의 쏠림을 나타내는 지표, 값이 클수록 가운데로 몰림( 뾰족한 모양, 평균에 가까운 값이 많다)
# skewness : 값의 쏠림정도, 음수라면 정규분포에서 오른쪽으로 쏠림, 양수라면 정규분포에서 왼쪽으로 쏠림
# nobs : # of oboservations (총 갯수)
# mean: 평균 , variance: 분산
print(stats.describe(product_per_user_li))

# 상품의 가짓수가 1이거나 600개 이상인 사용자는 일정 범위를 벗어나므로 군집화 데이터에서 제외시킬 예정
# 구매한 상품의 가짓수가 1인 사용자의 ID를 검색
min_product_user_li = [k for k,v in user_product_dic.items() if len(v)==1]
# 구매한 상품의 가짓수가 600개 이상인 사용자의 ID를 검색
...
# 찾아낸 사용자를 군집화에 사용할 user_product_dic 에서 제외(하고 user_product_dic을 재설정)
...
# 데이터 셋에서 제외된 사용자가 구매한 상품은 군집화에 사용하지 않기 때문에 이러한 처리를 해줌
id_product_dic = {}
