...
def search_info(choice_condition):
    ...
    for index in range(len(json_big_data)):
        ...
        if choice_condition==2: # student_ID
            ...
            if json_big_data[index]['student_age'] == in_search:
                found.append(index)
                found_count += 1
            ...
            if in_total['num_of_course_learned'] == in_search:
                found.append(index)
                found_count += 1
        ...
        if choice_menu==0:
            ...
            if choice_menu!=0:
                if len(in_total['learning_course_info'])>1:
                    choice_modify = input('여러 항목이 있습니다. 어떤 것을 변경하겠습니까?(값 입력)')
                    in_modify = input("변경할 값을 입력하세요: ")
                    for in_learn in in_total['learning_course_info']:
                        if choice_menu==1:
                    ...
                    for in_learn in in_total['learning_course_info']:
                        if choice_menu==1:
                           if in_learn['course_code'] == choice_modify:
                            ...
                            if in_learn['course_name'] == choice_modify:
                                in_learn['course_name'] = in_modify
                        elif choice_menu==3:
                            if in_learn['teacher'] == choice_modify:
                                in_learn['teacher'] = in_modify
                        elif choice_menu==4:
                            if in_learn['open_date'] == choice_modify:
                                in_learn['open_date'] = in_modify
                        elif choice_menu==5:
                            if in_learn['close_date'] == choice_modify:
